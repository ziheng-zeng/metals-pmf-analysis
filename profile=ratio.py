...
background_start_time = pd.Timestamp('2023-10-01 00:00:00', tz='US/Eastern')
background_end_time = pd.Timestamp('2023-10-07 23:59:59', tz='US/Eastern')
...
# List of metals to analyze against PM2.5
...
# Calculate Metal/PM2.5 ratios for each period
for metal in metals_of_interest:
    conc_col = [col for col in df.columns if f"{metal} " in col and 'uncert' not in col.lower()][0]

    # Background Metal/PM2.5 Ratio
    ratio_background = background_df[conc_col] / background_df['UG/M3']
    ...
    # Fireworks Metal/PM2.5 Ratio
    ratio_fireworks = fireworks_df[conc_col] / fireworks_df['UG/M3']
    ...
    # Wildfire Metal/PM2.5 Ratio
    ratio_wildfire = wildfire_df[conc_col] / wildfire_df['UG/M3']
...
all_ratios_df = pd.concat(ratios_background + ratios_fireworks + ratios_wildfire).reset_index(drop=True)
...
# Customize the plot
plt.title('Distribution of Metal/PM2.5 Ratios for Background, Fireworks, and Wildfire', fontsize=16)
plt.ylabel('Metal/PM2.5 Ratio', fontsize=14)
...
# Example of calculating z-scores for each metal and event combination
def calculate_z_scores(group):
    mean = group['Ratio'].mean()
    std = group['Ratio'].std()
    group['Z-Score'] = (group['Ratio'] - mean) / std
    return group

all_ratios_df = all_ratios_df.groupby(['Metal', 'Event']).apply(calculate_z_scores)

plt.figure(figsize=(14, 7))
sns.boxplot(
    x='Metal', y='Z-Score', hue='Event', data=all_ratios_df,
    palette={'Background': 'gray', 'Fireworks': 'blue', 'Wildfire': 'green'}, showfliers=False
)

plt.title('Normalized Distribution of Metal/PM2.5 Ratios for Background, Fireworks, and Wildfire', fontsize=16)
plt.ylabel('Z-Score of Metal/PM2.5 Ratio', fontsize=14)
plt.xlabel('Metal', fontsize=14)
plt.xticks(rotation=45)
plt.tight_layout()
plt.show()

all_ratios_df = all_ratios_df.reset_index(drop=True)
# Function to apply Min-Max scaling
def min_max_scale(group):
    min_val = group['Ratio'].min()
    max_val = group['Ratio'].max()
    group['Min-Max Scale'] = (group['Ratio'] - min_val) / (max_val - min_val)
    return group

# Apply Min-Max scaling
all_ratios_df = all_ratios_df.groupby(['Metal', 'Event'], as_index=False).apply(min_max_scale)

# Plot using seaborn for Min-Max scaled data
plt.figure(figsize=(14, 7))
sns.boxplot(
    x='Metal', y='Min-Max Scale', hue='Event', data=all_ratios_df,
    palette={'Background': 'gray', 'Fireworks': 'blue', 'Wildfire': 'green'}, showfliers=False
)

plt.title('Min-Max Scaled Distribution of Metal/PM2.5 Ratios for Background, Fireworks, and Wildfire', fontsize=16)
plt.ylabel('Min-Max Scale of Metal/PM2.5 Ratio', fontsize=14)
plt.xlabel('Metal', fontsize=14)
plt.xticks(rotation=45)
plt.tight_layout()
...
#         'Ratio'].dropna()
#     wildfire_values = all_ratios_df[(all_ratios_df['Metal'] == metal) & (all_ratios_df['Event'] == 'Wildfire')]['Ratio'].dropna()
#
...
# results_df.to_csv('D:/Documents/research-2024/Xact data/metal_PM2.5_ratio_test_results.csv', index=False)
# print(results_df)
